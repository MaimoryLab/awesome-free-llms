...
export default function Home() {
  ...
  return (
    <main className="site-shell">
      ...
      <section className="hero" aria-labelledby="page-title"><div className="hero__copy"><p className="kicker">OPEN SOURCE OF OPPORTUNITY</p><h1 id="page-title">发现正在发生的<br /><em>免费 AI</em> 福利。</h1><p className="hero__lede">汇总各家 LLM 的免费额度、代金券和长期计划。保持更新，按需领取。</p></div><div className="hero__aside"><span className="hero__count">{currentPagination?.total ?? "—"}</span><span>条线索<br />已收录</span></div></section>
      <section className="catalog" aria-label="免费活动目录">
        <div className="catalog__header"><div className="tabs" role="tablist" aria-label="活动类型">{tabs.map((tab) => <button key={tab.kind} className={`tab ${kind === tab.kind ? "is-active" : ""}`} role="tab" aria-selected={kind === tab.kind} onClick={() => selectKind(tab.kind)}><span>{tab.eyebrow}</span>{tab.label}</button>)}</div><Link className="submit-link" href="/submit">分享一条线索 <span aria-hidden="true">↗</span></Link></div>
